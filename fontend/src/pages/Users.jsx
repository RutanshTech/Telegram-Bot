...
const Users = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Fetch users with payment details
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      // Only add users that have an invoiceId
  ...
  // Download invoices for selected users as a ZIP file
  const handleDownloadSelectedInvoices = async () => {
    // Get invoice IDs for selected users
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto">
      ...
      {/* Top section: Search, Download Button */}
      <div className="flex flex-col sm:flex-row items-center justify-between mb-6 space-y-4 sm:space-y-0 sm:space-x-4">
        ...
        {/* Download Selected Invoices Button (always visible) */}
