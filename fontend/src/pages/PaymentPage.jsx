...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    // Check for payment status and transaction ID
  ...
  const calculateExpiryDate = (duration) => {
    ...
    if (typeof duration === 'string') {
      ...
      // If duration is a number, assume it's days
      expiryDate.setDate(now.getDate() + duration);
    } else {
      // Default to 30 days if duration is invalid
      expiryDate.setDate(now.getDate() + 30);
    }
  ...
  const convertDurationToDays = (duration) => {
    ...
    return typeof duration === 'number' ? duration : 30; // Default to 30 days
  };
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const requestData = {
        ...
        duration: durationInDays // Send number of days instead of string
      };
      ...
      localStorage.setItem('paymentDetails', JSON.stringify({
        ...
        duration: durationInDays, // Store number of days
        originalDuration: planData.duration // Store original duration format
      }));
