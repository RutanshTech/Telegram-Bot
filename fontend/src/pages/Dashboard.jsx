...
const Dashboard = () => {
  ...
  // New state for today's users count
  const [todayUsersCount, setTodayUsersCount] = useState(0);
  useEffect(() => {
    // Fetch today's users from backend
    axios.get('http://localhost:4000/api/kyc/today')
      .then(res => {
        console.log('Today users API response:', res.data); // Debug log
  ...
  // Net revenue state (real value from backend)
  ...
  // Total transactions state (real value from backend)
  ...
  // Active users state (real value from backend)
  ...
  // Real data for Bar chart (today's new users)
  const barData = {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <div className="pl-50 transition-all duration-300">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-6 md:px-12 pb-12">
          ...
          {/* Recent Transactions Table (replaces Pie chart) */}
