...
const DigioErrors = () => {
  ...
  useEffect(() => {
    fetchErrors();
    // Set up polling to check for new errors every 30 seconds
  ...
  const fetchErrors = async () => {
    try {
      ...
      console.log('First error user data:', response.data[0]?.userId); // Log first error's user data
      console.log('All user IDs:', response.data.map(e => e.userId)); // Log all user IDs
      ...
      console.error('Error fetching Digio errors:', err);
      if (err.response) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Errors Table - Only show when card is clicked or there are errors */}
