...
const OTP = () => {
  const [otp, setOtp] = useState(new Array(4).fill("")); // 4 inputs
  ...
  const handleChange = (el, index) => {
    ...
    if (/^\d$/.test(value)) {
      ...
      newOtp[index] = value;
      setOtp(newOtp);
      if (index < 3) inputRefs.current[index + 1].focus(); // move focus to next
    } else if (value === "") {
      const newOtp = [...otp];
      newOtp[index] = "";
      setOtp(newOtp);
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Check if we have a plan from location state
      if (plan) {
