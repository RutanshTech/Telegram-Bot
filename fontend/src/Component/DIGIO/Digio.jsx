...
export default function Digio() {
  ...
  // const [creditStatus, setCreditStatus] = useState(null); // Removed because backend route does not exist
  const navigate = useNavigate();

  useEffect(() => {
    // checkCreditStatus(); // Removed because backend route does not exist
  }, []);

  // Removed checkCreditStatus function because /api/digio/credit-status does not exist in backend
    ...
    try {
      ...
      const digio = new window.Digio({
        ...
        callback: async (response) => {
          console.log('Digio callback response:', response); // Debug log
          if (response.error_code) {
            ...
            localStorage.setItem('digioCompleted', 'true'); // For Steps.jsx, even on error
            setTimeout(() => {
              navigate('/'); // Debug: force redirect even on error
            }, 2000);
          } else {
            try {
              ...
              console.log('SignedRes:', signedRes); // Debug log
              setSignedUrl(signedRes.data.signedLink);
              toast.success('Document signed successfully!', {
                toastId: 'signing-success'
              });
              // Set digioCompleted in localStorage and redirect to home
              localStorage.setItem('digioCompleted', 'true'); // For Steps.jsx
              setTimeout(() => {
                navigate('/');
              }, 2000);
            } catch (err) {
              console.error('Error in signed-link callback:', err); // For Steps.jsx, even on error
              localStorage.setItem('digioCompleted', 'true'); // <-- Yahan bhi
              setTimeout(() => {
                navigate('/'); // Debug: force redirect even on error
              }, 2000);
            }
      ...
      if (errorMessage.includes('Insufficient credits')) {
        // Log the error but show a generic message to user
