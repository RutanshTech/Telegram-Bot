...
const NOTIFICATION_EXPIRY_DAYS = 7; // Notifications older than this will be removed
...
const Navbar = ({ title = "Admin Panel" }) => {
  ...
  useEffect(() => {
    ...
    socket.on('newUser', (user) => {
      ...
      if (!existingNotification) {
        const newNotification = {
          ...
          read: false,
          priority: 'high',
          userId: user._id || user.id // Add user ID to prevent duplicates
        };
    ...
    socket.on('payment', (data) => {
      const newNotification = {
        ...
        read: false,
        priority: 'high',
        paymentId: data.paymentId // Add payment ID to prevent duplicates
      };
      
      setNotifications(prev => [newNotification, ...prev].slice(0, MAX_NOTIFICATIONS));
    });

    // Only fetch initial notifications if we don't have any saved
  ...
  const fetchInitialNotifications = async () => {
    try {
      ...
      // Convert users to notifications, but only for recent users
